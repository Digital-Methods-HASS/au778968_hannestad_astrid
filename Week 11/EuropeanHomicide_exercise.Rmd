...
Western_Europe %>% 
  ...
  geom_line() +
  labs(
    title = "Density of Homicides per 100k people",
    x = "Year",
...
```{r Comparison to monarchs reign duration}
kings <- read_csv2( "data/Kongeraekken-csv.csv")

kings <- kings %>% 
  mutate(reign_duration = End_reign_year - Beginning_reign_year) %>% 
  mutate(midyear = Beginning_reign_year + reign_duration/2)
...
As we can see on the graphs, homicides per capita has been falling drastically. This is also confirmed by the graph showing the duration of monnarchs reign, where during the medieval period, the reigns are shorter. Specifically in Italy, we see the highest levels overall, probably due to general unrest, in a country that was plague by wars between different smaller states. As to whether we are more civilized today, that is a hard question, because civilization isn't necessarily defined by lack of violence. But what we can say, is that with the growing state, and other avenues to let out frustration other than murder. So in that regard we are more civilized more civilized today.
